...
def decipher(input):
    ...
    for edge in edges:                              #Instead of iritating 1000 times, we now continue until there is 1 large circuit
        if len(circuits) == 1:                      #which means size of circuits is 1
            break
        connect(edge,circuits)
        last_edge = edge

    output = 1                                      #Multiply 2 X coordinates of the last connection
    for box in last_edge:
        output *= input[box][0]
    return output
